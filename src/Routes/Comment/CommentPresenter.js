...
export default ({ 
    ...
    } else if (!loading  && !postLoading
                && data  && data.seeComment
                && postData  && postData.seeBuyOne) {
        const { seeBuyOne: { location, lastDate, applys, applysCount } } = postData;
        const { seeBuyOne: { user: { avatar, userName }} } = postData;
