...
import WriteRecruitMeGroupPresenter from "./WriteRecruitMeGroupPresenter";
import { CREATE_BUYME_GROUP, CREATE_CONTENTS, CONNECT_CONTENTS } from "./WriteRecruitMeGroupQueries";

export default ({ 
    match: { params: { groupRoomId }}, 
    location: { pathname }
}) => {
    const [action, setAction] = useState("view");
    const [createBuyMeMutation] = useMutation(CREATE_BUYME_GROUP);
    ...
    const createBuyMe = async() => {
        ...
        // GoogleMaps
        let location;
        if (pathname === `/writeRecruitMeGroup/${groupRoomId}` ||
             pathname === `/writeRecruitMeGroup/${groupRoomId}`) {
            return toast.error("지역을 작성 해주세요.");
        } else {
            const locationSplit = pathname.split(`/writeRecruitMeGroup/${groupRoomId}/`);
