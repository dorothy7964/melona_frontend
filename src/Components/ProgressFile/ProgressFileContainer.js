...
export default ({ changeId, categoryId, userName, anotherPage }) => {
  ...
  if (loading === true) {
    ...
    return categoryContents.map((contents) => (
      <div key={contents.id}>
        {!anotherPage ? (
          contents.confirmFile === "none" ? (
            <Wrapper>
              ...
              <TextContainer>
                <CategoryBox>#{contents.category.text}</CategoryBox>
                <ContentBox>{contents.text}</ContentBox>
              </TextContainer>
            </Wrapper>
          ) : (
            <Wrapper>
          ...
          contents.contentsReqs.map(
            (contentsReqs) =>
              contentsReqs.id === changeId && (
                <ConfirmFileBox key={contentsReqs.id}>
